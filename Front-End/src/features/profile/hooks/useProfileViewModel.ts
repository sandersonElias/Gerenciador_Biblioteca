...
import { EmprestimosAtivoDto, ReservaResponse } from "../models/ProfileModel";
import {
  EmprestimoService,
...
export const useProfileViewModel = () => {
  ...
  const [cancelTarget, setCancelTarget] = useState<ReservaResponse | null>(
    null,
  );
  ...
  const { data: meusEmprestimos = null } =
    useQuery<MeusEmprestimosResponse | null>({
  ...
  const solicitarRenovacaoMutation = useMutation({
    ...
    onSuccess: () => {
      showToast(
        "Solicitação de renovação enviada! Aguarde a aprovação.",
        "success",
      );
    ...
    onError: (err: any) => {
      showToast(
        err.response?.data?.message || "Erro ao solicitar renovação",
        "error",
      );
    },
  });
